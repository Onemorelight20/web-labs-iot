...
export const userWithSuchValuesExists = (values) => {
    ...
    if(emails.includes(receivedMail)){
        const users = getUsersAsJsonOrEmptyList();
        const foundUser = users.map((jsonUser) => JSON.parse(jsonUser)).filter((user) => user.email == receivedMail)[0];
        return foundUser == undefined ? false : foundUser.password == values.password;
    } else {
        return false;
    }
}
